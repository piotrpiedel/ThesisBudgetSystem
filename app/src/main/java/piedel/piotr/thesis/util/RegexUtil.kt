...
fun regexOneToTenDigitsCommaWhiteSpaceAndLetterA_D(): Regex = Regex("""\b\d{1,10}[,]\d{2}\s*(?i)[A-D]{1}\b""")

// (1-10 digits)(,)(one or more spaces)(one letter A-D)
fun regexOneToTenDigitsDotWhiteSpaceAndLetterA_D(): Regex = Regex("""\b\d{1,10}[.]\d{2}\s*(?i)[A-D]{1}\b""")

// (1-10 digits)(,|.)(3 digits)
fun regexOneToTenDigitsDotOrCommaWThreeDigits(): Regex = Regex("""\b\d{1,10}[.,]\d{3}\b""")
